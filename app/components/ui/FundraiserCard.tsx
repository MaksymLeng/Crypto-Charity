...
export default function FundraiserCard(props: FundraiserCardProps) {
    const { slug, title, coverUrl, currency, goalAmount, totalRaised } = props;
    const pct = Math.min(100, Math.round((totalRaised / Math.max(goalAmount, 1)) * 100));

    return (
        <Link
            href={`/event/${slug}`}
            className="group block rounded-2xl overflow-hidden border border-zinc-200 hover:shadow-lg transition duration-300 hover:-translate-y-1"
        >
            <div className="mt-4 flex items-center gap-3 px-4">
                ...
                <div className="sm:text-2xl font-semibold leading-tight">{title}</div>
            </div>
